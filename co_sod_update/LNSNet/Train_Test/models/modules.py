...
class CoAttLayer(nn.Module):
    ...
    def forward(self, x5):
        if self.training:
            ...
            x5_1_proto = x5_1_proto.unsqueeze(-1).unsqueeze(-1) # CoCA, C, CoCA, CoCA

            x5_2_proto = torch.mean(x5_new_2, (0, 2, 3), True).view(1, -1)
            x5_2_proto = x5_2_proto.unsqueeze(-1).unsqueeze(-1) # CoCA, C, CoCA, CoCA
            ...
            x5_proto = x5_proto.unsqueeze(-1).unsqueeze(-1) # CoCA, C, CoCA, CoCA
...
class GAM(nn.Module):
    ...
    def forward(self, x5):
        ...
        x_w = x_w.mean(-1)
        #x_w = torch.mean(x_w, -CoCA).values # BHW
        x_w = x_w.view(B, -1) * self.scale # B, HW
        x_w = F.softmax(x_w, dim=-1) # B, HW
        x_w = x_w.view(B, H5, W5).unsqueeze(1) # B, CoCA, H, W
