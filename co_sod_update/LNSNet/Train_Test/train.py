...
from Co_SOD.LNSNet.Train_Test.util import Logger, AverageMeter, save_checkpoint, save_tensor_img, set_seed
import os
import numpy as np
import argparse
...
# from codes.bayibest82segformerbest.best.distill.student.student2 import M as S
# from codes.bayibest82segformerbest.best.distill.teacher.ablation_定稿.ablation.backbone import M as S
# from codes.bayibest82segformerbest.best.distill.teacher.student0_weight import M as S
from Co_SOD.CADC.CoSODNet.CoSODNet_D import CoSODNet as S
...
parser.add_argument('--epochs', default=50, type=int)  # 320
...
parser.add_argument('--size',
                    ...
                    type=int,
                    help='input size')
parser.add_argument('--ckpt_dir', default="/storage/student4/wby_data/output", help='Temporary folder')
parser.add_argument('--ckpt_dir2', default="/media/wby/shuju/ckpt2", help='Temporary folder')
parser.add_argument('--testsets',
                    default='DUTS_class',  # default='', CoCA+CoSOD3k+Cosal2015    val5.0
                    type=str,
                    help="Options: 'CoCA','Cosal2015','CoSOD3k','iCoseg','MSRC'")

parser.add_argument('--val_dir',
                    default='/storage/student4/wby_data/tmp4val',
                    type=str,
...
if args.trainset == 'DUTS_class':
    ...
    train_loader = get_loader(train_img_path,
                              ...
                              num_workers=8,
                              pin=True)
    train_img_path_seg = os.path.join(root_dir, 'images/DUTS_class')
    train_gt_path_seg = os.path.join(root_dir, 'gts/DUTS_class')
    train_softgt_path_seg = os.path.join(root_dir, 'gts/DUTS_class')
    train_edge_path_seg = os.path.join(root_dir, 'gts/DUTS_class')
...
# model2.load_pre("/media/wby/shuju/seg_pre/segformer.b2.512x512.ade.160k.pth")
# model2.load_pre("/home/wby/segformer.b5.640x640.ade.160k.pth")
# model2.load_pre("/storage/student4/wby_data/pretrain_weights/vgg16_20M.caffemodel.pth")
load_model = '/storage/student4/wby_data/pretrain_weights/vgg16_20M.caffemodel.pth'
vgg_model = torch.load(load_model)
net = model2.init_parameters(vgg_model)
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        #     kl = (kl1 + kl2 + kl3)*2
            # kl4 = CE(return_values[6], gts)
        # kl5 = kld(dte[0], teacher_rte[0], gts, 3)
        # kl6 = kld(dte[1], teacher_rte[1], gts, 3)
        # b3 addition !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # b3l = CE(res1, gts)
        # print(kl)
        # student
        ...
        cls_percentage = F.adaptive_avg_pool2d(gts, 8)
        # loss = 0
        # print(return_values[0].size())
        # loss1 = CE(return_values[0], gts)
        # loss2 = CE(return_values[1], gts)
        # loss3 = CE(return_values[2], gts)
        loss6 = CE(return_values[-1], gts)
        # loss_sal = (loss1 + loss2 + loss3 + loss6)*10
        loss_sal = loss6 * 10
        # ------------------------------------------------
        ...
        # print(b3l, kl*100, (loss_sal1 + loss_sal2 + loss_sal3) *10)
        # loss_sal = (loss_sal1 + loss_sal2 + loss_sal3) *10 + kl * 100 + loss_sal4 + b3l
