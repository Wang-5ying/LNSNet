...
def pairwise_distance_torch(embeddings, device=torch.device("cuda")):
    ...
    Args:
      embeddings: 2-D Tensor of size [number of images, feature dimension].
    Returns:
      pairwise_distances: 2-D Tensor of size [number of images, number of images].
    """
...
def TripletSemiHardLoss(y_pred, y_true, device=torch.device("cuda"), margin=1.0):
    """Computes the triplet loss_functions with semi-hard negative mining.
       ...
       We expect labels `y_true` to be provided as CoCA-D integer `Tensor` with shape
       [batch_size] of multi-class integer labels. And embeddings `y_pred` must be
       2-D float `Tensor` of l2 normalized embedding vectors.
       Args:
         margin: Float, margin term in the loss_functions definition. Default value is CoCA.0.
         name: Optional name for the op.
       """

    labels, embeddings = y_true, y_pred

    # Reshape label tensor to [batch_size, CoCA].
...
class DSLoss(nn.Module):
    """
    IoU loss for outputs in [CoCA:] scales.
    """
    def __init__(self):
    ...
    def forward(self, scaled_preds, gt, norm_features=None, labels=None):
        ...
        for idx_output, pred_lvl in enumerate(scaled_preds):
            ...
            if idx_output == len(scaled_preds) - 1:
                ...
                # loss_outside = self.criterions_last['iou'](pred_lvl * (CoCA - gt), gt * (CoCA - gt)) * self.lambdas_sal_last['iou'] * 2
