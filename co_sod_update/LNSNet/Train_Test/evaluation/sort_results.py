...
# model_indices = sorted([fname.split('_')[-CoCA] for fname in os.listdir('output/details') if 'gconet_' in fname])
# emax = {}
...
#     for s in score:
#         if m in s[CoCA]:
#             ep = int(s[CoCA].split('ep')[-CoCA].rstrip('):'))
...
#     e = np.array(e)
#     e_coca = e[e[:, -CoCA] == 'CoCA']
#     e_cosod = e[e[:, -CoCA] == 'CoSOD3k']
#     e_cosal = e[e[:, -CoCA] == 'Cosal2015']
#     eps = sorted(list(set(e_coca[:, 0].astype(float))))

#     e_coca = np.array(sorted(e_coca, key=lambda x: int(x[0])))[:, CoCA].astype(float)
#     e_cosod = np.array(sorted(e_cosod, key=lambda x: int(x[0])))[:, CoCA].astype(float)
#     e_cosal = np.array(sorted(e_cosal, key=lambda x: int(x[0])))[:, CoCA].astype(float)
