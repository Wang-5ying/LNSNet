...
from backbone.Shunted.SSA import shunted_t as mit_b5
...
class M(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.resnet = mit_b5()
        self.am = AM2(64, 256, 64, 128)
        self.u4 = nn.Conv2d(96, 1, 1)
        self.gm1 = GM2(64, 64, 96)
        self.gm2 = GM2(256, 256, 24)

        self.am_d = AM2(64, 256, 64, 128)
        self.u4_d = nn.Conv2d(96, 1, 1)
        self.gm1_d = GM2(64, 64, 96)
        self.gm2_d = GM2(256, 256, 24)

        self.sup1 = BasicConv2d(256 * 2, 1, 3, 1, 1)
        ...
        self.bound2 = nn.Conv2d(256, 1, 1)
        self.bound3 = nn.Conv2d(64, 1, 1)
        self.bound4 = nn.Conv2d(256, 1, 1)

        self.node1 = node(64, 128, 80, 40)
        self.node2 = node(256, 512, 20, 10)
    ...
    def forward(self, r, d):
        ...
        # distill
        if self.training:
            return res2, res1, res, rd1, rd3, rte, dte#, wr1, wd1, wr2, wd2 ####726
        else:
...
if __name__=='__main__':
    ...
    b = torch.randn(2, 1, 320, 320).cuda()
    model=M()
    model.cuda()
    out=model(a, b)
    flops, par = profile(model, (a, b))
    print(flops /1e9, par / 1e6)
    # for i in out:
#         print(i.shape)
