...
class M(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.u4 = nn.Conv2d(32, 3, 1)
        # self.gm1 = GM2(512, 512, 8)

        self.sup1 = BasicConv2d(160, 1, 3, 1, 1)
        self.sup2 = BasicConv2d(64, 1, 3, 1, 1)
        self.sup3 = BasicConv2d(32, 1, 3, 1, 1)

        # change to B
        # self.node1 = node(64, 128, 64, 32)
        # self.node2 = node(320, 512, 16, 8)

        self.ta1 = Channel_aware_CoordAtt(32, 32, 64, 64)
        self.ta2 = Channel_aware_CoordAtt(64, 64, 32, 32)
        self.ta3 = Channel_aware_CoordAtt(160, 160, 16, 16)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.classifier = nn.Linear(512, 291)

        self.bc1 = nn.Conv2d(256, 160, 3, 1, 1)
        self.bc2 = nn.Conv2d(256, 64, 3, 1, 1)
        self.bc3 = nn.Conv2d(256, 32, 3, 1, 1)

        # self.gm1 = CAAM(512, 1, [4, 4], norm_layer=nn.BatchNorm2d)
        self.gm1_c = BasicConv2d(256, 256, 4, 2, 1, 1)
        self.gm1 = CAAM_WBY(256, 1, [32, 32], norm_layer=nn.BatchNorm2d)
        self.gm1_2 = CAAM_WBY(160, 1, [32, 32], norm_layer=nn.BatchNorm2d)
        self.gm1_3 = CAAM_WBY(64, 1, [64, 64], norm_layer=nn.BatchNorm2d)
        self.cam2 = BasicConv2d(160, 160, 1)
        self.cam3 = BasicConv2d(64, 64, 1)


        self.beforeca1 = BasicConv2d(32, 64, 3, 2, 1)  # 往pre_decoder里加do_conv效果会变差
        self.ca1 = CA(64)
        self.ca1_1 = AFF(64)
        # self.ca1 = AFF(128)
        self.beforeca2 = BasicConv2d(64, 160, 3, 2, 1)
        # self.ca2 = CA(320)
        self.ca2 = AFF(64)
        self.ca2_2 = AFF(64)
        self.beforeca3 = BasicConv2d(160, 256, 3, 2, 1)
        # self.ca3 = CA(512)
        self.ca3 = AFF(160)
        self.ca3_3 = AFF(160)
        self.beforeca4 = BasicConv2d(256, 256, 3, 1, 1)
        # self.ca4 = CA(512)
        self.ca4 = AFF(256)
        self.ca4_4 = AFF(256)

        self.md1 = Bottleneck2D(256, 256)
        self.md2 = Bottleneck2D(160, 160)
        self.md3 = Bottleneck2D(64, 64)
        self.md4 = Bottleneck2D(32, 32)

        self.mr1 = Bottleneck2D(256, 256)
        self.mr2 = Bottleneck2D(160, 160)
        self.mr3 = Bottleneck2D(64, 64)
        self.mr4 = Bottleneck2D(32, 32)
        ...
        self.dp1 = Dp(256, 160, 160, 16)
        self.dp2 = Dp(160, 64, 64, 32)
        self.dp3 = Dp(64, 32, 32, 64)
        ...
        self.bot4_1 = Bottleneck2D(32, 32)
        self.bot4 = Bottleneck2D(32, 32)

        self.trans8 = torch.nn.ConvTranspose2d(32, 1, 3, 2, 1, 1, 1, True, 1)

        # decoder
        self.deco1 = torch.nn.ConvTranspose2d(256, 160, 3, 2, 1, 1, 1, True, 1)
        self.deco2 = torch.nn.ConvTranspose2d(160, 64, 3, 2, 1, 1, 1, True, 1)
        self.deco3 = torch.nn.ConvTranspose2d(64, 32, 3, 2, 1, 1, 1, True, 1)
        ...
        # return flow
        self.before1 = BasicConv2d(256, 160, 3, 1, 1)
        self.r1 = BasicConv2d(160, 64, 3, 1, 1)
        self.before2 = BasicConv2d(160, 64, 3, 1, 1)
        self.r2 = BasicConv2d(64, 32, 3, 1, 1)
        self.before3 = BasicConv2d(64, 32, 3, 1, 1)
        self.r3 = BasicConv2d(32, 3, 3, 1, 1)

        self.before1t = BasicConv2d(256, 160, 3, 1, 1)
        self.t1 = BasicConv2d(160, 64, 3, 1, 1)
        self.before2t = BasicConv2d(160, 64, 3, 1, 1)
        self.t2 = BasicConv2d(64, 32, 3, 1, 1)
        self.before3t = BasicConv2d(64, 32, 3, 1, 1)
        self.t3 = BasicConv2d(32, 3, 3, 1, 1)

        self.zd1 = nn.Conv2d(160, 320, 1)
        self.zd2 = nn.Conv2d(64, 128, 1)
        self.zd3 = nn.Conv2d(32, 64, 1)
        self.zd4 = nn.Conv2d(32, 64, 1)
