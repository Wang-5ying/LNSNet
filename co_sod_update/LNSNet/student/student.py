...
from Co_SOD.LNSNet.mix_transformer import mit_b2
# from mmseg.models import build_segmentor
...
class M(nn.Module):
    ...
    def forward(self, r, d, temperature):
        ...
        res1 = res1 * guide
        B = res1.shape[0]
        x, H, W = self.resnet.patch_embed4(res1)
        ...
        res2 = res2 * guide
        x, H, W = self.resnet.patch_embed3(res2)
        for i, blk in enumerate(self.resnet.block3):
            x = blk(x, H, W)
        x = self.resnet.norm3(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        ...
        x, H, W = self.resnet.patch_embed2(res3)
        for i, blk in enumerate(self.resnet.block2):
            x = blk(x, H, W)
        x = self.resnet.norm2(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        ...
        # stage 1
        x, H, W = self.resnet.patch_embed1(res4)
        for i, blk in enumerate(self.resnet.block1):
            x = blk(x, H, W)
        x = self.resnet.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
