from Co_SOD.CADC.ImageBranchEncoder import ImageBranchEncoder
from Co_SOD.CADC.ImageBranchDecoder import ImageBranchDecoder
import torch.nn as nn
import torch
from torch.nn import BatchNorm2d as bn
from Co_SOD.CADC.Modules import KernelModule
from Co_SOD.CADC.Modules import Transformer
from Co_SOD.CADC.Modules import poolingModule
...
class CoSODNet(nn.Module):
    def load_pre(self, pre_model1):
        ...
        self.ImageBranchEncoder.load_state_dict(new_state_dict3, strict=True) # True
        # self.ImageBranchDecoder.load_state_dict(new_state_dict3, strict=False)
    ...
    def __init__(self, n_channels, mode='train'):
        ...
        # self.ImageBranchEncoder_d = ImageBranchEncoder(n_channels)
    ...
    def forward(self, image_Input, D):
        if self.mode == 'train':
            preds = self._train_forward(image_Input, D)
        else:
            preds = self._test_forward(image_Input, D)

        return preds

    def _train_forward(self, image_Input, D):
        outputs_image = self._forward(image_Input, D)
        return outputs_image

    def _test_forward(self, image_Input, D):

        with torch.no_grad():
            outputs_image = self._forward(image_Input)
            return outputs_image

    def _forward(self, image_Input, Depth):
        N, _, _, _ = image_Input.size()
        image_feas = self.ImageBranchEncoder(image_Input)
        # image_feas_d = self.ImageBranchEncoder(Depth)
        # for i in image_feas_d:
        #     print("i", i.size())
...
if __name__ == "__main__":
    ...
    inp1 = torch.randn((2, 3, 256, 256)).cuda()
    inp2 = torch.randn((2, 3, 256, 256)).cuda()
    out = model(inp1, inp2)
