...
parser.add_argument('--ckpt_dir',
                    default="/storage/student4/wby_data/comparemethod_output/CADC/1",
...
parser.add_argument('--val_dir',
                    default='/storage/student4/wby_data/comparemethod_output/CADC/1/tmp4val/',
                    type=str,
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        loss2 = CE(F.sigmoid(F.interpolate(return_values[-2], scale_factor=2, mode='bilinear', align_corners=True)), gts)
        loss3 = CE(F.sigmoid(F.interpolate(return_values[-3], scale_factor=4, mode='bilinear', align_corners=True)),
                   gts)
        loss4 = CE(F.sigmoid(F.interpolate(return_values[-4], scale_factor=8, mode='bilinear', align_corners=True)),
                   gts)
        loss5 = CE(F.sigmoid(F.interpolate(return_values[-5], scale_factor=8, mode='bilinear', align_corners=True)),
                   gts)
        loss6 = CE(F.sigmoid(F.interpolate(return_values[-6], scale_factor=8, mode='bilinear', align_corners=True)),
                   gts)
        ...
        loss = 0
        # loss_sal = 0
        loss_sal = loss1 + loss2 + loss3 + loss4 + loss5 + loss6 #+ loss2# + loss1_b + loss2_b
