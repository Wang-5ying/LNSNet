...
class Eval_thread():
    def __init__(self, loader, method='', dataset='', output_dir='', epoch='', cuda=True):
        ...
        self.dataset2smeasure_bottom_bound = {'CoCA': 0.673, 'CoSOD3k': 0.802, 'Cosal2015': 0.845, 'RGBD_CoSal150': 0.875, "RGBD_CoSal1k": 0.875, "RGBD_CoSeg183":0.700}      # S_measures of GCoNet_plus
    ...
    def run(self, AP=False, AUC=False, save_metrics=False, continue_eval=True):
        ...
        if continue_eval:
            ...
            if s > self.dataset2smeasure_bottom_bound[self.dataset] -0.1:
                mae = self.Eval_mae()
        ...
        return '[cost:{:.4f}s] '.format(time.time() - start_time) + info, continue_eval, max_e, s, max_f, mae
