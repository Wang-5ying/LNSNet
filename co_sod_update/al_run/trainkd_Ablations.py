...
from COA_RGBD_SOD.al.models.New_Test_Shunted_T_Project import RGBD_sal
...
parser.add_argument('--size',
                    ...
                    type=int,
                    help='input size')
parser.add_argument('--ckpt_dir', default="/home/map/Alchemist/COA/COA_RGBD_SOD/ckpt/Ablation_KD_Model/New_Test_Shunted_T_Project/Pth1",
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.epochs - 8 or measures[0] >= 0.8490:
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        t_s_3 = 1 - t_s_3
        t_s_4 = 1 - t_s_4
        # T_dynamic = a * T_S_1 + b * T_S_2 + c * T_S_3 + d * T_S_4
        # T_F_dynamic = F_a * T_F_1 + conv2_1(up2(F_b * T_F_2)) + conv3_1(up4(F_c * T_F_3)) + conv4_1(up8(F_d * T_F_4))
        ...
        if epoch <= 30:
            ...
            # loss_dynamic = KLDLoss1(S_S_1 / 4, T_dynamic / 4, gts, 5)
            loss_final = los1_8 * 4 + los2_8 + los3_8 + los4_8# + loss_dynamic * 2
            ...
            # loss_F_dynamic = AT_Loss(S_F_1 / temperature, T_F_dynamic / temperature, gts, 4)

            loss_midel = loss_F_1 * 4 + loss_F_2 + loss_F_3 + loss_F_4# + loss_F_dynamic * 2
            ...
            # loss_dynamic = KLDLoss1(S_S_1 / 1, T_dynamic / 1, gts, 5)
            loss_final = los1_8 * 4 + los2_8 + los3_8 + los4_8# + loss_dynamic * 2
            loss_kd = loss_final * (1 - 0.0625) + 0.0625 * loss_gt
            ...
            # loss_F_dynamic = AT_Loss(S_F_1 / 1, T_F_dynamic / 1, gts, 4)
            loss_midel = loss_F_1 * 4 + loss_F_2 + loss_F_3 + loss_F_4# + loss_F_dynamic * 2
            loss_midel = loss_midel
            ...
            # loss_dynamic = KLDLoss1(S_S_1 / temperature, T_dynamic / temperature, gts, 5)
            loss_final = los1_8 * 4 + los2_8 + los3_8 + los4_8# + loss_dynamic * 2
            loss_kd = loss_final * (1 - 0.0625) + 0.0625 * loss_gt
