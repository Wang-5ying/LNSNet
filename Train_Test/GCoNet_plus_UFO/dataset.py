...
class CoData(data.Dataset):
    def __init__(self, image_root, label_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root,
    ...
    def __getitem__(self, item):
        ...
        for idx in range(final_num):
            ...
            if self.is_train:
                if 'flip' in Config().preproc_methods:
                    image, label, softgt, softgt2, softgt3, depth, edge = cv_random_flip(image, label, softgt, softgt2,
                                                                                         softgt3, depth, edge)
                if 'crop' in Config().preproc_methods:
                    image, label, softgt, softgt2, softgt3, depth, edge = random_crop(image, label, softgt, softgt2,
                                                                                      softgt3, depth, edge)
                if 'rotate' in Config().preproc_methods:
                    image, label, softgt, softgt2, softgt3, depth, edge = random_rotate(image, label, softgt, softgt2,
                                                                                        softgt3, depth, edge)
            ...
            image, label, softgt, softgt2, softgt3, depth, edge = self.transform_image(image), self.transform_label(
                label), self.transform_softgt(softgt), self.transform_softgt2(softgt2), self.transform_softgt2(
                softgt2), self.transform_depth(depth), self.transform_edge(edge)
            images[idx] = image
            labels[idx] = label
            softgts[idx] = softgt
        ...
        if self.is_train:
            ...
            return images, labels, softgts, softgt2s, softgt3s, depths, edges, subpaths, ori_sizes, cls_label
...
def get_loader(img_root, gt_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root, img_size, batch_size,
               max_num=float('inf'), istrain=True, shuffle=False,
               num_workers=1, pin=False):
    # print(depth_root,"jhhhhh")
    dataset = CoData(img_root, gt_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root, img_size,
