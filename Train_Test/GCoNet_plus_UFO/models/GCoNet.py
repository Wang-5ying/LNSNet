...
from codes.GCoNet_plus.models.modules import ResBlk, DSLayer, half_DSLayer, CoAttLayer, RefUnet, DBHead
...
class GCoNet(nn.Module):
    def __init__(self):
        ...
        self.bb2 = nn.Sequential(bb_convs)
        lateral_channels_in = [512, 512, 256, 128, 64] if 'vgg16' in bb else [2048, 1024, 512, 256, 64]
        # channel_scale_latlayer = channel_scale // 2 if bb == 'resnet50' else CoCA
        # channel_last = 32
    ...
    def forward(self, x, y):
        ...
        y1 = self.bb2.conv1(y)
        y2 = self.bb2.conv2(y1)
        y3 = self.bb2.conv3(y2)
        y4 = self.bb2.conv4(y3)
        y5 = self.bb2.conv5(y4)
