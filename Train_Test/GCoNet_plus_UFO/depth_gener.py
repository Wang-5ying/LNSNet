...
from codes.bayibest82segformerbest.best.distill.student.student2_b1 import M
...
def main(args):
    ...
    pretrained_dict = dd.io.load(args.ckpt)
    new_pre_dict = {}
    for k,v in pretrained_dict.items():
        new_pre_dict[k] = torch.Tensor(list(map(len, v)))
    ...
    for testset in args.testsets.split('+'):
        ...
        for batch in tqdm(test_loader):
            ...
            ori_sizes = batch[-1]
            with torch.no_grad():
                scaled_preds = model(inputs, depths)[-1]  # 取得模型的最后一个输出作为预测结果
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size',
                        ...
                        type=int,
                        help='input size')
    parser.add_argument('--ckpt', default='/media/wby/KINGSTON/kitti.h5', type=str,  # 71-8627
                        help='model folder')
