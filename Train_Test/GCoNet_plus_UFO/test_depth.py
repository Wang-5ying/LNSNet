...
def main(args):
    ...
    # load pth
    base_weights = args.ckpt
    new_state_dict3 = OrderedDict()
    state_dict = torch.load(base_weights)['model']
    for k, v in state_dict.items():
        name = k[7:]
        new_state_dict3[name] = v
    model.load_state_dict(new_state_dict3)
    model.eval()
    ...
    for testset in args.testsets.split('+'):
        ...
        for batch in tqdm(test_loader):
            ...
            ori_sizes = batch[-1]
            with torch.no_grad():
                scaled_preds = model(inputs)  # 取得模型的最后一个输出作为预测结果
...
if __name__ == '__main__':
    ...
    parser.add_argument('--size',
                        ...
                        type=int,
                        help='input size')
    parser.add_argument('--ckpt', default='/media/wby/shuju/ckpt2/best_ep117_Smeasure0.8694.pth', type=str,  # 71-8627
                        help='model folder')
